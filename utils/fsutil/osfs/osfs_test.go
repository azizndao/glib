...
func TestOSFS(t *testing.T) {
	...
	t.Run("OpenFile", func(t *testing.T) {
		fs := &FS{}
		file, err := fs.OpenFile("resources/test_file.txt", os.O_RDONLY, 0o660)
		require.NoError(t, err)
		defer func() {
			assert.NoError(t, file.Close())
		}()
	...
	t.Run("Mkdir", func(t *testing.T) {
		...
		require.NoError(t, fs.Mkdir(path, 0o770))
	...
	t.Run("MkdirAll", func(t *testing.T) {
		...
		require.NoError(t, fs.MkdirAll(path, 0o770))
		assert.True(t, fs.IsDirectory(path))
	})
	...
	t.Run("Remove", func(t *testing.T) {
		...
		assert.NoError(t, fs.MkdirAll(path, 0o770))
		assert.True(t, fs.IsDirectory(path))
