...
func (g *Generator) generateDI() (string, error) {
	...
	needsFmt := len(g.project.Providers) > 0   // Only need fmt if there are providers (for error formatting)
	needsHTTP := len(g.project.Middleware) > 0 // Only need http if there's middleware
